...
elseif strcmp(method,'ai')
    ...
    for i = 1:Ngp                   % loop through significant GTO products    
        ...
        if ri(i,:)==rj(i,:)
            Fat = Fat + 2*pre0(i)*ex1;    % atomic terms
        else
            Fmol = Fmol + 2*pre0(i)*ex1;  % molecular terms
        end
    end % end loop over Gaussian products
    
else
    disp('METHOD not valid: choose either "iam" or "ai"')
end % end method if
%========================================
